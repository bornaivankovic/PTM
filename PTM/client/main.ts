    ...
    $(document).on('click', '.calculate-abraham', function () {
        ...
        if (startNode != 'Network' && endNode != 'Network') {
            ...
            for (let nodepair of result) {
                let resultPath = nodepair.pair.split(",");
                let startNodeTitle = resultPath[0];
                let endNodeTitle = resultPath[1];
    ...
    $(document).on('click', '.calculate', function () {
        ...
        if (startNode != 'Network' && endNode != 'Network') {
            ...
            for (let result of globalResultDijkstra.responseJSON.result) {
                let resultPath = result.primary.path.split("-");
                let startNodeTitle = resultPath[0];
                let endNodeTitle = resultPath[resultPath.length-1];
